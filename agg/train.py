...
from agg.data_utils import SentenceREDataset, get_idx2tag, load_checkpoint, save_checkpoint
from agg.model import SentenceRE
...
def train(hparams,BT,BM):
    ...
    if test_file:
        ...
        with torch.no_grad():
            ...
            recall = metrics.recall_score(tags_true, tags_pred, average='weighted')
            accuracy = metrics.accuracy_score(tags_true, tags_pred)
            print("agg_test recall:",recall)
            print("agg_test precision:",precision)
            print("agg_test f1:",f1)
