...
from op.data_utils import SentenceREDataset, get_idx2tag, load_checkpoint, save_checkpoint
from op.model import SentenceRE
...
def train(hparams,BT,BM):
    ...
    if test_file:
        ...
        with torch.no_grad():
            ...
            recall = metrics.recall_score(tags_true, tags_pred, average='weighted')
            accuracy = metrics.accuracy_score(tags_true, tags_pred)
            print("op_test recall:",recall)
            print("op_test precision:",precision)
            print("op_test f1:",f1)
