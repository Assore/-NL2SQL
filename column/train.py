...
from column.data_utils import SentenceREDataset, get_idx2tag, load_checkpoint, save_checkpoint
...
def train(hparams,BT,BM):
    ...
    for epoch in range(epoch_offset, epochs):
        ...
        model.train()
        #bert的token_type_ids划分句子，token_ids，句子中的token，attention_mask避免填充token影响结果
        for i_batch, sample_batched in enumerate(tqdm(train_loader, desc='Training')):
            ...
            loss = criterion(logits.float(), tag_ids.float())
            loss.backward()
            optimizer.step()
        ...
        if validation_file:
            ...
            with torch.no_grad():
                ...
                tags_true=np.array(tags_true).reshape(-1)
                tags_pred=np.array(tags_pred).round().reshape(-1)
                # accuracy=metrics.roc_auc_score(tags_true,tags_pred)
    ...
    if test_file:
        ...
        with torch.no_grad():
            ...
            print("column_test recall:",recall)
            print("column_test precision:",precision)
            print("column_test f1:",f1)
