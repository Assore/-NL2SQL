...
def get_tag2idx(file):
    ...
    return dict((tag, idx+1) for idx, tag in enumerate(tagset))
...
class SentenceREDataset(Dataset):
    def __init__(self, data_file_path, tagset_path, pretrained_model_path=None, max_len=128,Tokenizer=None):
        ...
        self.tag_list = get_tag2idx(self.tagset_path)
    ...
    def __getitem__(self, idx):
        ...
        for tag in self.tag_list:
            if tag in sample_tag:
                sample_tag_id.append(1)
            else:
                sample_tag_id.append(0)
        ...
        sample = {
            ...
            'tag_id':torch.tensor(np.array(sample_tag_id,dtype=float))
        }
        return sample
